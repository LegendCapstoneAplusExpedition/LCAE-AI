...
def build_pipeline(stt_config: PipelineConfig, tts_config: TTSConfig, topics: list[str] | None = None, on_synthesis=None, broadcast_id: str = ""):
    ...
    def emit_opening() -> None:
        """AI 진행자 합류 직후, 방송 주제 기반 오프닝 멘트를 1회 발화한다.

        streaming_stage="Opening" 상태에서 그래프를 한 번 돌리면 generate_opening
        노드가 멘트를 만들고 stage를 Main으로 전이시킨다(이후 재호출돼도 발화 없음).
        """
        if state.get("streaming_stage") != "Opening":
            return
        opening_result = llm_app.invoke(state)
        state.update(opening_result)

        msgs = state.get("messages", [])
        last_msg = msgs[-1] if msgs else None
        if isinstance(last_msg, AIMessage) and last_msg.content.strip():
            print(f"[LLM] ◀ 오프닝 멘트: \"{last_msg.content}\"")
            ai_outputs.append(
                mentor_text="",
                mentor_confidence=1.0,
                state=state,
                ai_text=last_msg.content,
                spoken=True,
            )
