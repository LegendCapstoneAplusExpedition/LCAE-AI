        ...
        try:
            ...
            if previous_summary:
                ...
                prompt = (
                    ...
                    "- 청취자에게 들려주듯, 각 문장을 '~라는 이야기를 했어요', '~라는 대화를 나눴네요' 같은 부드러운 구어체 어미로 끝맺으세요.\n"
                    "- 핵심만 1~2문장으로 짧게, 다른 설명 없이 요약문만 출력하세요."
                )
            ...
            system_prompt = (
                "당신은 방송 진행자가 청취자에게 지금까지의 내용을 들려주는 요약기입니다. "
                "입력된 전사 내용만 바탕으로, 핵심만 짧게 간추린 한국어 평문 요약을 작성하세요. "
                "각 문장은 '~라는 이야기를 했어요', '~라는 대화를 나눴네요', '~를 짚어봤어요' 같은 "
                "부드러운 구어체 어미로 끝맺습니다. JSON·키-값·대괄호 태그·코드블록·머리말 없이, "
                "요약 문장 자체만 출력합니다."
            )
